		...
		if(ep_events[i].data.fd == lb_serv_sock){//Packet From Client
			...
			for(int i = 0; i < table_connection_cnt; i++){ //Looking for whether it exists nor.
				if( (strcmp(table[i].clnt_IP, clnt_ip) == 0) && (table[i].clnt_PORT == ntohs(tcp_header->source)) ){
					printf("clnt IP: %s, Port:%hu\n",table[i].clnt_IP, table[i].clnt_PORT);
				       	already_connected= 1;
				       	current_table_idx = i;
				       	break;
				} 
			...
			if(tcp_header->syn == 1){
				...
				memcpy(&target_serv, &serv_info[selected_serv_idx], sizeof(struct serv_info));//TODO: MEMORY ERROR I GUESS!!
				printf("%s\n",target_serv.serv_IP);
				...
				memcpy(&target_table, &table[current_table_idx], sizeof(struct table)); 		
