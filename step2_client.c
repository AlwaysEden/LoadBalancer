    ...
    */
	...
	for(i = 0; i < event_cnt; i++){
		if(ep_events[i].data.fd == lb_serv_sock){//Packet From Client
			...
			memcpy(checksum_header+sizeof(struct pseudo_header), tcp_header, tcp_header->doff*4);
			memcpy(checksum_header+sizeof(struct pesudo_header)+(tcp_header->doff*4), buffer+(ip_header->ihl*4+ tcp_header->doff*4), packet_len-ip_header->ihl*4-tcp_header->doff*4);
			...
			memset(checksum_header, 0, psize);
			memcpy(checksum_header, &p_header, sizeof(struct pseudo_header));
			memcpy(checksum_header+sizeof(struct pseudo_header), tcp_header, tcp_frame_size);
